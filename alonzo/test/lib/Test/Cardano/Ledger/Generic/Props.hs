...
module Test.Cardano.Ledger.Generic.Props where
...
pp :: Witness era -> Core.PParams era
pp wit =
  newPParams
    wit
...
utxoEnv :: Witness era -> UtxoEnv era
utxoEnv wit =
  UtxoEnv
    (SlotNo 0)
    (pp wit)
...
scriptAddr :: forall era. Reflect era => Core.Script era -> Witness era -> Addr (Crypto era)
...
feeKeys :: forall era. Era era => Witness era -> KeyPair 'Payment (Crypto era)
feeKeys _wit = KeyPair vk sk
  where
    (sk, vk) = mkKeyPair @(Crypto era) (0, 0, 0, 0, 1)

feeAddr :: forall era. Reflect era => Witness era -> Addr (Crypto era)
...
feeOutput :: Reflect era => Witness era -> Core.TxOut era
feeOutput wit =
  newTxOut Override wit [Address $ feeAddr wit, Amount (inject $ Coin 1000)]

initUTxO :: (Parametric era) => Witness era -> UTxO era
initUTxO wit =
  UTxO $
    Map.fromList
...
alwaysSucceedsOutput :: forall era. (Parametric era) => Witness era -> Core.TxOut era
...
outEx1 :: Reflect era => Witness era -> Core.TxOut era
outEx1 wit = newTxOut Override wit [Address (feeAddr wit), Amount (inject $ Coin 5995)]

validatingBody :: Reflect era => Witness era -> Core.TxBody era
validatingBody wit =
  newTxBody
    Override
    wit
    [ Inputs [TxIn genesisId 0],
      Inputs_fee [TxIn genesisId 2],
      Outputs [outEx1 wit],
      Txfee (Coin 5),
...
utxoEx1 :: forall era. Parametric era => Witness era -> UTxO era
utxoEx1 wit =
  UTxO $
    Map.fromList
      [ (TxIn genesisId 1, alwaysFailsOutput wit),
...
alwaysFailsOutput :: forall era. (Scriptic era, Reflect era) => Witness era -> Core.TxOut era
...
outEx2 :: Reflect era => Witness era -> Core.TxOut era
outEx2 wit = newTxOut Override wit [Address (feeAddr wit), Amount (inject $ Coin 3995)]

notValidatingBody :: Reflect era => Witness era -> Core.TxBody era
...
notValidatingTx :: forall era. (Parametric era) => Witness era -> Core.Tx era
notValidatingTx wit =
  newTx
    Override
    wit
...
utxoEx2 :: forall era. Parametric era => Witness era -> UTxO era
utxoEx2 wit =
  UTxO $
    Map.fromList
      [ (TxIn genesisId 0, alwaysSucceedsOutput wit),
