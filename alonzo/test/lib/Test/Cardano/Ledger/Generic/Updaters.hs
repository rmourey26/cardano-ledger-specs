...
import Test.Cardano.Ledger.Generic.EraWitness
...
class Era era => Scriptic era where
  always :: Natural -> Witness era -> (Core.Script era)
  never :: Natural -> Witness era -> (Core.Script era)
  require :: KeyHash 'Witness (Crypto era) -> Witness era -> (Core.Script era)
  allOf :: [Witness era -> (Core.Script era)] -> Witness era -> (Core.Script era)
  anyOf :: [Witness era -> (Core.Script era)] -> Witness era -> (Core.Script era)
  mOf :: Int -> [Witness era -> (Core.Script era)] -> Witness era -> (Core.Script era)

class Scriptic era => PostShelley era where
  before :: Int -> Witness era -> Core.Script era
  after :: Int -> Witness era -> Core.Script era
...
unTime :: CC.Crypto (Crypto era) => Witness era -> (Witness era -> Script era) -> Timelock (Crypto era)
...
matchkey :: Scriptic era => Int -> Witness era -> Core.Script era
matchkey n era = require (theKeyHash n) era

test21 :: Scriptic era => Witness era -> Core.Script era
test21 wit = allOf [always 1, matchkey 1, anyOf [matchkey 2, matchkey 3]] $ wit

test22 :: PostShelley era => Witness era -> Core.Script era
...
initialTxBody :: Era era => Witness era -> Core.TxBody era
...
initialWitnesses :: Era era => Witness era -> Core.Witnesses era
...
initialTx :: forall era. Witness era -> Core.Tx era
...
initialPParams :: forall era. Witness era -> Core.PParams era
...
updateTx :: Policy -> Witness era -> Shelley.Tx era -> TxField era -> Shelley.Tx era
...
newTx :: Policy -> Witness era -> [TxField era] -> Core.Tx era
...
updateTxBody :: Policy -> Witness era -> Core.TxBody era -> TxBodyField era -> Core.TxBody era
updateTxBody p (Shelley _) tx dt = case dt of
  (Inputs is) -> tx {Shelley._inputs = applyMerge p (Shelley._inputs tx) is}
  (Inputs_fee is) -> tx {Shelley._inputs = applyMerge p (Shelley._inputs tx) is}
...
updateTxBody p (Allegra _) tx@(MA.TxBody ins outs certs wdrls txfee vldt ups adHash mint) dt = case dt of
  ...
  (Inputs_fee is) -> MA.TxBody (applyMerge p (MA.inputs' tx) is) outs certs wdrls txfee vldt ups adHash mint
...
updateTxBody p (Alonzo _) tx dt = case dt of
  ...
  (Inputs_fee is) -> tx {Alonzo.txinputs_fee = applyMerge p (Alonzo.txinputs_fee tx) is}
...
newTxBody :: Era era => Policy -> Witness era -> [TxBodyField era] -> Core.TxBody era
...
updateWitnesses :: forall era. Policy -> Witness era -> Core.Witnesses era -> WitnessesField era -> Core.Witnesses era
...
newWitnesses :: Era era => Policy -> Witness era -> [WitnessesField era] -> Core.Witnesses era
...
updatePParams :: Witness era -> Core.PParams era -> PParamsField era -> Core.PParams era
...
newPParams :: Witness era -> [PParamsField era] -> Core.PParams era
...
applyValue :: Policy -> Witness era -> Core.Value era -> Core.Value era -> Core.Value era
...
updateTxOut :: Policy -> Witness era -> Core.TxOut era -> TxOutField era -> Core.TxOut era
...
newTxOut :: Era era => Policy -> Witness era -> [TxOutField era] -> Core.TxOut era
...
-- | A Meaningless Addr.
initialAddr :: Era era => Witness era -> Addr (Crypto era)
...
initialTxOut :: Era era => Witness era -> Core.TxOut era
...
vkey :: Era era => Int -> Witness era -> VKey 'Witness (Crypto era)
vkey n _w = theVKey n
